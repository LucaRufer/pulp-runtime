/*
 * Copyright (C) 2023 ETH Zurich, University of Bologna
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */


#ifndef __ARCHI_CHIPS_SAFETY_ISLAND_MEMORY_MAP_H__
#define __ARCHI_CHIPS_SAFETY_ISLAND_MEMORY_MAP_H__

#define ARCHI_L2_SHARED_ADDR 0x10000000
#define ARCHI_L2_SHARED_SIZE 0x01000000

#define ARCHI_SAFETY_ISLAND_BASE_ADDR 0x00000000
#define ARCHI_SAFETY_ISLAND_ADDR_RANGE 0x10000000
#define ARCHI_SAFETY_ISLAND_PERIPH_OFFSET 0x01000000
#define ARCHI_SAFETY_ISLAND_L1_OFFSET 0x04000000
#define ARCHI_SAFETY_ISLAND_DMEM_OFFSET 0x0C000000

#define ARCHI_L1_SIZE 0x00010000
#define ARCHI_DMEM_SIZE 0x00004000
#define ARCHI_BOOT_ROM_SIZE 0x00010000
#define ARCHI_IMEM_SIZE 0x00004000

/*
 * MEMORIES
 */

#define ARCHI_SAFETY_ISLAND_END_ADDR ( ARCHI_SAFETY_ISLAND_BASE_ADDR + ARCHI_SAFETY_ISLAND_ADDR_RANGE)

#define ARCHI_L1_ADDR       ( ARCHI_SAFETY_ISLAND_BASE_ADDR + ARCHI_SAFETY_ISLAND_L1_OFFSET )
#define ARCHI_DMEM_ADDR     ( ARCHI_SAFETY_ISLAND_BASE_ADDR + ARCHI_SAFETY_ISLAND_DMEM_OFFSET )
#define ARCHI_IMEM_ADDR     ( ARCHI_SAFETY_ISLAND_END_ADDR - ARCHI_IMEM_SIZE )
#define ARCHI_BOOT_ROM_ADDR ( ARCHI_IMEM_ADDR - ARCHI_BOOT_ROM_SIZE )

#define ARCHI_LOCAL_PRIV0_ADDR  ARCHI_L1_ADDR
#define ARCHI_LOCAL_PRIV0_SIZE  ARCHI_L1_SIZE

// L2 alias
#define ARCHI_L2_PRIV0_ADDR  ARCHI_L2_SHARED_ADDR
#define ARCHI_L2_PRIV0_SIZE  (ARCHI_L2_SHARED_SIZE / 2)

#define ARCHI_L2_PRIV1_ADDR  (ARCHI_L2_PRIV0_ADDR + ARCHI_L2_PRIV0_SIZE)
#define ARCHI_L2_PRIV1_SIZE  (ARCHI_L2_SHARED_SIZE / 2)

/*
 * PERIPHERALS
 */

#define ARCHI_SAFETY_ISLAND_PERIPHERALS_ADDR    ( ARCHI_SAFETY_ISLAND_BASE_ADDR + ARCHI_SAFETY_ISLAND_PERIPH_OFFSET )

#define ARCHI_SOC_CTRL_OFFSET       0x00000000
#define ARCHI_CLIC_OFFSET           0x01000000
#define ARCHI_INSTR_ERR_OFFSET      0x01010000
#define ARCHI_DATA_ERR_OFFSET       0x01010010
#define ARCHI_SHADOW_ERR_OFFSET     0x01010020
#define ARCHI_FC_TIMER_OFFSET       0x02000000
#define ARCHI_STDOUT_OFFSET         0x02FF0000

#define ARCHI_SOC_CTRL_ADDR         ( ARCHI_SAFETY_ISLAND_PERIPHERALS_ADDR + ARCHI_SOC_CTRL_OFFSET )
#define ARCHI_CLIC_ADDR             ( ARCHI_SAFETY_ISLAND_PERIPHERALS_ADDR + ARCHI_CLIC_OFFSET )
#define ARCHI_INSTR_ERR_ADDR        ( ARCHI_SAFETY_ISLAND_PERIPHERALS_ADDR + ARCHI_INSTR_ERR_OFFSET )
#define ARCHI_DATA_ERR_ADDR         ( ARCHI_SAFETY_ISLAND_PERIPHERALS_ADDR + ARCHI_DATA_ERR_OFFSET )
#define ARCHI_SHADOW_ERR_ADDR       ( ARCHI_SAFETY_ISLAND_PERIPHERALS_ADDR + ARCHI_SHADOW_ERR_OFFSET )
#define ARCHI_FC_TIMER_ADDR         ( ARCHI_SAFETY_ISLAND_PERIPHERALS_ADDR + ARCHI_FC_TIMER_OFFSET )
#define ARCHI_STDOUT_ADDR           ( ARCHI_SAFETY_ISLAND_PERIPHERALS_ADDR + ARCHI_STDOUT_OFFSET )

#define ARCHI_FC_TIMER_SIZE            0x00000800

#endif
